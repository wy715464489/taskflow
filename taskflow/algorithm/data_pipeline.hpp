...
namespace tf
{
  ...
  */
  ...
  // constructor
  template <typename... Ps>
  DataPipeline<Ps...>::DataPipeline(size_t num_lines, std::tuple<Ps...> &&ps) : _pipes{std::forward<std::tuple<Ps...>>(ps)},
                                                                                _meta{_gen_meta(
  ...
  void DataPipeline<Ps...>::_on_pipe(Pipeflow &pf, Runtime &)
  {
    ...
    else if constexpr (std::is_invocable_v<callable_t, std::decay_t<typename data_pipe_t::input_t>&>) {
    ...
    else if constexpr (std::is_invocable_v<callable_t, std::decay_t<typename data_pipe_t::input_t>&, Pipeflow&>) {
